...
#
#     def add(self, output, target):
#         output = output.squeeze()                        # 删除所有维数为1的维度 Confusion.add
#         target = target.squeeze()
#         assert output.size(0) == target.size(0), \
#                 'number of targets and outputs do not match'
...
#             pred = output
#         indices = (target*self.conf.stride(0) + pred.squeeze_().type_as(target)).type_as(self.conf)   # self.conf.stride(0)：8  就是矩阵self.conf的宽度
#         # print(target*self.conf.stride(0))
#         # print(pred.squeeze_().type_as(target))
#         # print("where:",indices)
#         ones = torch.ones(1).type_as(self.conf).expand(indices.size(0))
#         # print(ones)
#         self._conf_flat = self.conf.view(-1)
